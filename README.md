...
The app uses an XGBoost regressor trained on 250,000 job records (200k
train / 50k held-out test) with:

- **Feature Engineering**: Non-linear experience terms, pairwise interactions, experience-level bins
- **Algorithm**: XGBoost (500 trees, max_depth=7, learning_rate=0.05)
- **Target**: raw annual salary in USD (no log transform at inference time)
...
XGBoost was selected over Linear Regression, Random Forest and
Gradient Boosting baselines on R². See `notebook.ipynb` for the full
leaderboard, 5-fold CV, residual analysis and feature importances.

## Files

- `app.py`: Main Streamlit application
- `best_salary_model_improved.joblib`: Trained ML model (generated by the notebook)
